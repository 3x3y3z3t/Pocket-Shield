...
namespace PocketShield
{
    ...
    public class ItemCard
    {
        ...
        private Vector2 DefIconUvOffset
        {
            get
            {
                return new Vector2((DefTextureSlot % Constants.TEXTURE_W) * Constants.TEXTURE_UV_SIZE_X,
                                   (DefTextureSlot / Constants.TEXTURE_H) * Constants.TEXTURE_UV_SIZE_Y);
            }
        }
        private Vector2 ResIconUvOffset
        {
            get
            {
                return new Vector2((ResTextureSlot % Constants.TEXTURE_W) * Constants.TEXTURE_UV_SIZE_X,
                                   (ResTextureSlot / Constants.TEXTURE_H) * Constants.TEXTURE_UV_SIZE_Y);
            }
        }
        ...
        public ItemCard()
        {
            ...
            m_DefIcon = new HudAPIv2.BillBoardHUDMessage()
            {
                ...
                uvEnabled = true,
                uvSize = new Vector2(Constants.TEXTURE_UV_SIZE_X, Constants.TEXTURE_UV_SIZE_Y),
                Blend = BlendTypeEnum.PostPP,
                Options = HudAPIv2.Options.Pixel
            };
        ...
        public void UpdateItemCardConfig()
        {
            ...
            m_Separator.Offset = new Vector2D(Constants.MARGIN * config.ItemScale, 0.0);
            m_Separator.Width = 5.0f * config.ItemScale;
            m_Separator.Height = Height;

            m_DefIcon.Visible = Visible;
            m_DefIcon.Origin = Origin + Offset;
            m_DefIcon.Offset = new Vector2D(22.0 * ConfigManager.ClientConfig.ItemScale, 0.0);
            m_DefIcon.Width = c_IconSize * config.ItemScale;
            m_DefIcon.Height = c_IconSize * config.ItemScale;

            m_ResIcon.Visible = Visible;
            m_ResIcon.Origin = Origin + Offset;
            m_ResIcon.Offset = new Vector2D(70.0 * ConfigManager.ClientConfig.ItemScale, 0.0);
            m_ResIcon.Width = c_IconSize * config.ItemScale;
            m_ResIcon.Height = c_IconSize * config.ItemScale;


            m_DefLabel.Visible = Visible;
            m_DefLabel.Origin = Origin + Offset;
            m_DefLabel.Scale = c_TextScale * config.ItemScale;

            m_ResLabel.Visible = Visible;
            m_ResLabel.Origin = Origin + Offset;
            m_ResLabel.Scale = c_TextScale * config.ItemScale;
        ...
        private void AlignLabels()
        {
            ...
            m_DefLabel.Offset = new Vector2D(center - halfW + 1.0, 35.0 * ConfigManager.ClientConfig.ItemScale);

            halfW = (float)m_ResLabel.GetTextLength().X * 0.5f;
            center = (float)m_ResIcon.Offset.X + c_IconSize * 0.5f;
            m_ResLabel.Offset = new Vector2D(center - halfW + 1.0, 35.0 * ConfigManager.ClientConfig.ItemScale);
        }
